...
for(n in c(1:number_of_series)){
  ...
  fake_table <- as.data.frame(fake_table)
  # label it and the cols
  fake_table[3] <- paste0("dummySNOMEDconcept", n)
  colnames(fake_table) <- c("year", "cases", "codename")
  head(fake_table)
  listofseries[[n]] <- fake_table
...
library(ggplot2)
 ...
 ggplot(multipleCODEtable,  aes(x=year, y= cases, group=codename, color=codename)) +
    geom_line() + expand_limits(y=0)
...
ggplot(multipleCODEtable, aes(x=year, y=cases, fill= codename)) + 
    geom_area() +
...
###4) Introducing different starting points


Some series start long ago (like 1990) whereas some only started in the last 4 years. We want to see if we can build this characteristic into our loop


It's easier to demonstrate this by going back to our simpler chunk from step 2. We're going to sample from a set of values
...
```
If you keep running the above it'll keep picking at random a year for the data to start from

So we incorporate this into our loop now
...
### 6) generating 'n' , 'n_id' and 'n_id_distinct'

The three measures given for concepts/clusters are 'n' , 'n_id' and 'n_id_distinct'.

- n means how many applicable records there were.

- n_id means how many applicable records there were that has an individual tied to it.

- n_id_distinct means how many different individuals have at least one record.

Seems like all the records had an id attached, when looking at the actual data, so we'll mimic that.

n_id_distinct must be lower than n, and is fairly proportional to the overall volume. So we'll randomly determine what proportion it is.
